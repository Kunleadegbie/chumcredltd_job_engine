...
gov_compare_df = pd.DataFrame({
    ...
    "Score": [
        ...
        float(national_avg_score) if 'national_avg_score' in locals() else 0
    ]
}).set_index("Metric")
...
hiring_trend = (
    supabase_admin.table("institution_employer_hiring_trend")
    .select("month,hires_count")
    .eq("institution_id", selected_inst_id)
    .order("month")
    .execute()
    .data or []
...
if hiring_trend:
    df_hire = pd.DataFrame(hiring_trend)
    st.line_chart(df_hire.set_index("month")["hires_count"])

salary_trend = (
    supabase_admin.table("institution_salary_trend")
...
if salary_trend:
    df_salary = pd.DataFrame(salary_trend)
    st.line_chart(df_salary.set_index("month")["avg_salary"])
...
tier_trend = (
    supabase_admin.table("institution_intelligence_snapshot")
    .select("reporting_year,public_tier")
    .eq("institution_id", selected_inst_id)
    .order("reporting_year")
    .execute()
    .data or []
...
if tier_trend:
    df_tier = pd.DataFrame(tier_trend)
    st.write(df_tier)
